  ...
  public criarTabelas(db: SQLiteObject) {
    ...
    return new Promise((resolve, reject) => {
      db.transaction(tx => {
        ...
        promisesTx.push(new Promise((resolve, reject) => { tx.executeSql('CREATE TABLE IF NOT EXISTS veiculos (Id integer primary key AUTOINCREMENT NOT NULL, Placa TEXT NOT NULL, Modelo TEXT, TipoVeiculo integer NOT NULL, Entrada DATE NOT NULL, Saida DATE, Telefone TEXT, Nome TEXT, Observacoes TEXT, Servicos TEXT, EntregaAgendada integer, PrevisaoEntrega Date, Funcionario TEXT, Localizacao TEXT, Ativo integer)', [], () => resolve(), (erro) => reject(erro))} ))
        promisesTx.push(new Promise((resolve, reject) => { tx.executeSql('CREATE TABLE IF NOT EXISTS veiculosHistorico (Id integer primary key AUTOINCREMENT NOT NULL, Placa TEXT, TipoVeiculo integer, IdFuncionario integer, Funcionario TEXT, Valor REAL, Descontos REAL, Acrescimos REAL, Entrada Date, Saida Date, Pagamento Date)', [], () => resolve(), (erro) => reject(erro))} ))
        ...
        promisesTx.push(new Promise((resolve, reject) => { tx.executeSql('CREATE TABLE IF NOT EXISTS movimentosServicos (IdMovimento integer NOT NULL, IdServico integer NOT NULL, Nome TEXT NOT NULL, Valor REAL NOT NULL, Desconto REAL, Acrescimo REAL, PRIMARY KEY (IdMovimento, IdServico))', [], () => resolve(), (erro) => reject(erro))} ))
        promisesTx.push(new Promise((resolve, reject) => { tx.executeSql('CREATE TABLE IF NOT EXISTS mensalistas (Id integer primary key AUTOINCREMENT NOT NULL, Nome TEXT, Documento TEXT, Telefone TEXT, Email TEXT, Ativo integer, Veiculos TEXT)', [], () => resolve(), (erro) => reject(erro))} ))
        promisesTx.push(new Promise((resolve, reject) => { tx.executeSql('CREATE TABLE IF NOT EXISTS funcionarios (Id integer primary key AUTOINCREMENT NOT NULL, Nome TEXT, Documento TEXT, Telefone TEXT, Email TEXT)', [], () => resolve(), (erro) => reject(erro))} ))
