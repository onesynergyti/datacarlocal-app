  ...
  public criarTabelas(db: SQLiteObject) {
    ...
    return new Promise((resolve, reject) => {
      db.transaction(tx => {
        ...
        promisesTx.push(new Promise((resolve, reject) => { tx.executeSql('CREATE TABLE IF NOT EXISTS mensalistas (Id integer primary key AUTOINCREMENT NOT NULL, Nome TEXT, Documento TEXT, Valor REAL, Telefone TEXT, Email TEXT, Ativo integer, Veiculos TEXT)', [], () => resolve(), (erro) => reject(erro))} ))
        promisesTx.push(new Promise((resolve, reject) => { tx.executeSql('CREATE TABLE IF NOT EXISTS pagamentoMensalistas (IdMovimento integer NOT NULL, IdMensalista integer NOT NULL, Inicio Date, Fim Date)', [], () => resolve(), (erro) => reject(erro))} ))
